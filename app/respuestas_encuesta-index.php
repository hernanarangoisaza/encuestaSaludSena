            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="page-header clearfix">
                        <h2 class="float-left">Respuestas de la encuesta Details</h2>
                        <a href="respuestas_encuesta-create.php" class="btn btn-success float-right">Add New Record</a>
                        <a href="respuestas_encuesta-index.php" class="btn btn-info float-right mr-2">Reset View</a>
                        <a href="index.php" class="btn btn-secondary float-right mr-2">Back</a>
                    </div>

                    <div class="form-row">
                        <form action="respuestas_encuesta-index.php" method="get">
                        <div class="col">
                    ...
                    $total_pages_sql = "SELECT COUNT(*) FROM respuestas_encuesta";
                    $result = mysqli_query($link,$total_pages_sql);
                    $total_rows = mysqli_fetch_array($result)[0];
                    $total_pages = ceil($total_rows / $no_of_records_per_page);
                    
                    //Column sorting on column name
                    $orderBy = array('idRespuestaEncuesta', 'idEncuesta', 'idPreguntaEncuesta', 'respuestaSiNo', 'estado', 'auditoria'); 
                    $order = 'idRespuestaEncuesta';
                    ...
                    if($result = mysqli_query($link, $sql)){
                        if(mysqli_num_rows($result) > 0){
                            ...
                            echo "<table class='table table-bordered table-striped'>";
                                echo "<thead>";
                                    echo "<tr>";
                                        echo "<th><a href=?search=$search&sort=&order=idRespuestaEncuesta&sort=$sort>Id Respuesta</th>";
                                ...
                                while($row = mysqli_fetch_array($result)){
                                    echo "<tr>";
                                    echo "<td>" . $row['idRespuestaEncuesta'] . "</td>";echo "<td>" . $row['idEncuesta'] . "</td>";echo "<td>" . $row['idPreguntaEncuesta'] . "</td>";echo "<td>" . $row['respuestaSiNo'] . "</td>";echo "<td>" . $row['estado'] . "</td>";echo "<td>" . $row['auditoria'] . "</td>";
                                        echo "<td>";
                                            echo "<a href='respuestas_encuesta-read.php?idRespuestaEncuesta=". $row['idRespuestaEncuesta'] ."' title='View Record' data-toggle='tooltip'><i class='far fa-eye'></i></a>";
                                            echo "<a href='respuestas_encuesta-update.php?idRespuestaEncuesta=". $row['idRespuestaEncuesta'] ."' title='Update Record' data-toggle='tooltip'><i class='far fa-edit'></i></a>";
                                            echo "<a href='respuestas_encuesta-delete.php?idRespuestaEncuesta=". $row['idRespuestaEncuesta'] ."' title='Delete Record' data-toggle='tooltip'><i class='far fa-trash-alt'></i></a>";
                                        echo "</td>";
                                    echo "</tr>";
                                }
                                echo "</tbody>";
